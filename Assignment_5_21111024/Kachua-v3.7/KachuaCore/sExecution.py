...
def generateConditions(s,pcIndex,pc,params,coverage,ir,pcEval):
    ...
    for i in range(0,len(coverage)):
        ...
        if irIndexPC==irIndexCoverage or isinstance(ir[irIndexCoverage][0], kachuaAST.ConditionCommand):
...
def symbolicExecutionMain(ir, params, constparams, timeLimit=10):
    ...
    while time.time() <= endTime:
        ...
        s.initProgramContext(params)
        s.resetSolver()
        pc, pcIndex =generateConditions(s,pcIndex,pc,params,coverage,ir,pcEval)
        res = s.s.check()
        print(s.s.assertions())
        print("res ",res)
        if str(res)=="sat":
